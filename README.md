...
当前正式版：`1.9.5`。第一次了解本项目，可以先阅读[软件介绍](软件介绍.md)。
...
- `luckcaty-cut-dota2_1.9.5_x64-setup.exe`：推荐，双击后按提示安装。

安装版启动后会静默检查官方 Release。发现新版本时只提示，是否下载和安装由用户决定；软件不会后台自动下载，也不会强制更新。`1.6.0` 用户需要先手动安装一次 1.7 或更高版本，之后才可使用应用内更新。

官方 1.9.5 安装包包含已核验的 FFmpeg/FFprobe 参考构建，许可证和来源记录见 `tools/ffmpeg/`。Git 仓库不提交这些二进制；自行构建时需要按“开发”章节提供本地 FFmpeg。Release 说明会列出 SHA-256 供核对。
...
- Dota 2 不在常见 Steam 目录时，可在设置中选择本机 `dota2.exe`；路径按电脑单独保存，也可恢复自动检测。
...
- “默认剪辑”和“WTF 导演”是两套独立方案，可以随时切换，采用 WTF 故事不会覆盖默认剪辑。
- 默认剪辑的“主角”页负责英雄与高光内容；WTF 导演的“故事”页把有证据的精彩操作、趣味互动和明确失误组织为剧情节拍。
- 导演按剧情决定机位：普通场次可只有玩家视角，关键动作才增加英雄跟随备用素材。
- 同一场次的机位使用相同时间码，按 `S001-A`、`S001-B` 编号导出独立 MP4。
- 默认成片只使用主机位；备用特写不会被顺序追加造成重复时长。
- 每个英雄击杀连续段可独立选择默认短击杀或剧情时长；剧情时长可保留击杀前 0 到 90 秒，并固定保留末杀后 10 秒。
- 相邻同英雄剧情段重叠时会缩短后一段前置范围，避免默认成片重复；切回短击杀可恢复原入点和出点。
- 增删、复制、排序片段并精确编辑 `HH:MM:SS.mmm` 时间，支持 30 FPS 单帧微调。
- “机位”页只设置当前片段：玩家视角为默认，`Hero Chase` 准确显示为“英雄跟随”。
- 普通画面/干净画面始终显示在工作台顶部；普通画面隐藏右上角观战播放、倍速和进度模块并保留游戏 HUD，干净画面隐藏完整 HUD 与鼠标指针。
- 最近任务完整显示并按最后打开时间刷新。
- 成片沿用原录像文件名；重复导出自动追加 `_2`、`_3`，不覆盖旧结果。
