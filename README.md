...
当前正式版：`1.9.8`。第一次了解本项目，可以先阅读[软件介绍](软件介绍.md)。
...
- `luckcaty-cut-dota2_1.9.8_x64-setup.exe`：推荐，双击后按提示安装。

安装版启动后会静默检查官方 Release。发现新版本时只提示，是否下载和安装由用户决定；软件不会后台自动下载，也不会强制更新。`1.6.0` 用户需要先手动安装一次 1.7 或更高版本，之后才可使用应用内更新。

官方 1.9.8 安装包包含已核验的 FFmpeg/FFprobe 参考构建，许可证和来源记录见 `tools/ffmpeg/`。Git 仓库不提交这些二进制；自行构建时需要按“开发”章节提供本地 FFmpeg。Release 说明会列出 SHA-256 供核对。

## 功能

### 录像分析

- 选择、拖入 `.dem`，或设置录像目录后通过比赛编号快速加载。
- 在本地解析比赛信息、10 人英雄阵容和整局事件，无需启动 Dota 2。
- 可导出 `evidence.json` 和 `report.md` 分析包，供后续视频制作读取。

### 高光剪辑

- 选择主角与高光内容，整理击杀、技巧互动、团战和地图目标。
- 支持默认剪辑与 WTF 导演，并可恢复完整候选、调整剧情时长和精确时间。
- 支持新增、复制、删除和排序片段。

### 镜头与画面

- 每段可选择玩家视角或英雄跟随，同场多机位使用统一编号和时间码。
- 可选择普通画面或干净画面；普通画面保留游戏 HUD，只隐藏右上角回放控制模块。

### 视频导出

- 使用 Dota 2 原生录像画面生成视频，不依赖 OBS。
- 成片沿用录像名称，重复导出自动编号，不覆盖已有文件。
- 导出后可直接播放视频或打开成片和源素材目录。

## 1.9.8 简介

- 新增录像分析包导出，生成 `evidence.json` 和 `report.md`。
- 修正分析包助攻统计，并优化默认保存位置。
...
- 修复普通画面仍显示右上角观战播放、倍速和进度模块的问题。
- 录制管线版本参与缓存指纹，画面控制升级后不会继续复用旧片段。

## 1.9.4 简介

- 每个英雄击杀连续段可独立切换“短击杀（默认）/剧情时长”。
- 剧情模式默认保留首杀前 60 秒，可调为 0 到 90 秒，并固定保留末杀后 10 秒。
- 相邻剧情段自动避免重复范围，旧剪辑方案可继续载入。

## 1.9.3 简介

- 增加按电脑保存的 Dota 2 程序路径配置，支持输入、浏览选择和恢复自动检测。
- 路径保存后立即刷新本机成片能力，不改变分析、镜头或导出流程。

## 1.9.2 简介

- 普通画面与干净画面改为始终可见的分段选择。
- 最近任务显示全部返回任务，并按最后打开时间刷新。
- 成片使用原录像名；重复导出自动添加 `_2`、`_3`，不覆盖已有文件。
...
- 新增可逆的“默认剪辑 / WTF 导演”双方案工作区，两套片段与设置独立保存。
- 将右侧交互拆分为“主角、时间、机位”，WTF 导演额外提供“故事”页。
- 旧版单方案任务自动迁移到默认剪辑，不丢失已有片段和精确时间。
- 玩家视角继续作为默认机位，`Hero Chase` 更名为准确的“英雄跟随”。
...
3. 在顶部选择“默认剪辑”或“WTF 导演”；两套方案来回切换时互不覆盖。
4. 默认剪辑在“主角”页选择英雄和内容；WTF 导演在“故事”页采用有证据的方案。
5. 核对 `S001-A/S001-B` 等素材编号，在固定画面选择中切换普通/干净画面，并在“时间”页为每个击杀段单独选择短击杀或剧情时长，再到“机位”页调整当前片段。
6. 点击“导出视频”复核预计文件名、画面模式、源素材数量、成片时长和游戏原声。
7. 点击“开始导出视频”。渲染期间不要操作 Dota 2。
8. 完成后在应用内播放，或打开成片和源素材目录。
